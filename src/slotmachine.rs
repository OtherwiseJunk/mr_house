...
fn check_pay_pattern_match(symbols: &[Symbol], pattern: &PayPattern) -> bool {
    match pattern {
        PayPattern::FiveOfAKind(symbol) => {
            symbols.iter().all(|&s| s == *symbol)
        }
        PayPattern::ThreeOfAKind(symbol) => {
            symbols.iter().filter(|&&s| s == *symbol).count() >= 3
        }
        PayPattern::MinCountAnyDistribution(symbols_list, min_count) => {
            let total = symbols_list.iter().map(|&s| {
                symbols.iter().filter(|&&sym| sym == s).count()
            }).sum::<usize>();
...
fn single_spin(
    ...
    let mut generated_symbols: Vec<Symbol> = (0..5)
        .map(|_| {
            *weighted_symbol_pool.get(rng.random_range(0..weighted_symbol_pool.len())).unwrap()
        })
        .collect();
...
impl SlotMachine {
    pub fn new(
        ...
        let min_jackpot = pay_table.iter()
        ...
        symbol_map.insert(Symbol::Magnathonk, "<:magnathonk:928158272836472872>".to_string());
        symbol_map.insert(Symbol::Smugbrow, "<:smugbrow:1013536644621664268>".to_string());
        symbol_map.insert(Symbol::Smileyes, "<:smileyes:927806099661422613>".to_string());
    ...
    fn get_symbol_string(&self, symbol: Symbol) -> String {
        self.symbol_map.get(&symbol).cloned().unwrap_or_else(|| "Unknown".to_string())
    }
}